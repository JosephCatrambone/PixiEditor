...
internal class BlurSharpenToolExecutor : UpdateableChangeExecutor
{
    ...
    public override ExecutionState Start()
    {
        ...
        IBlurSharpenToolHandler? blurTool = GetHandler<IBlurSharpenToolHandler>();
        IPenToolbar? toolbar = blurTool?.Toolbar as IPenToolbar;
        penToolbar = toolbar;
        IColorsHandler? colorsHandler = GetHandler<IColorsHandler>();

        if (colorsHandler is null || blurTool is null || member is null || toolbar is null)
        ...
        guidValue = member.Id;
        color = GetHandler<IColorsHandler>().SecondaryColor;
        ...
        IAction? action = new LineBasedPen_Action(guidValue, Colors.White, controller!.LastPixelPosition, blurTool.ToolSize, true,
